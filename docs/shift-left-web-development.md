...
At its core, web development is about making suggestions to the user agent regarding how a page should be experienced by the user (it's not about pixel-perfect accuracy; it never has been). The aim is to enable adaptation to a specific user's context, including their device, settings, browser and OS defaults, assistive technology and viewport size. For us, context is everything, and all of these factors are part of that context, just as much as the context of a stack of components nested into each other or site areas.

The beauty of this paradigm is that we develop our system as declarative boilerplate, and developers consuming our library bring their own context. A public-facing app might need broad browser support; a business portal might enforce a single browser. Tools like PostCSS or LightningCSS can transform our declarative CSS to match each consumer's browser targets; not including polyfills and downlevel syntax they don't need at all. JS workarounds, by contrast, always ship to everyone regardless, adding dead weight and probably even incompatibility in contexts where native support already exists.
...
- **Testability**: Native HTML/CSS behavior doesn't need testing, browsers already guarantee it. Less custom JS means fewer tests to write and maintain, faster CI pipelines, and no timing-dependent assertions (no `setTimeout` races, no `waitForTransition` flakiness). Tests become deterministic snapshots rather than fragile async state machines.
- **Fewer bugs**: Every line of JavaScript is an edge case waiting to happen. Browsers have already solved countless interaction edge cases (focus order, scroll restoration, animation cancellation, RTL, reduced motion). Native features handle these implicitly; custom JS reimplementations inevitably miss some, leading to bugs that frustrate users.
- **Regression safety**: Custom JavaScript implementations of features like e.g. positioning, invokers, or popovers are hard to regression-test because they interact with layout, focus, scroll, and resize in ways that are difficult to enumerate. Native platform features (Anchor Positioning, Invoker Commands, Popover API) are tested by browser vendors across millions of edge cases we could never cover ourselves.
- **Framework portability**: HTML and CSS work identically across all JS frameworks and through Mitosis code generation without adaptation. Any JS that touches the DOM (refs, event listeners, timers, observers) must be translated and tested for each framework output; React, Angular, Vue, and Web Components all handle lifecycle, refs, and cleanup differently. This is a constant source of subtle bugs and maintenance overhead as well as unknown unknowns regarding edge cases. Declarative HTML/CSS avoids the problem entirely.
- **Polyfills benefit consuming developers too**: Rather than baking functionality into our own JS implementations, suggesting or shipping polyfills (e.g. for Anchor Positioning, Invoker Commands, Popover API) gives consumers control — they can drop the polyfill once their browserslist covers native support. More importantly, consumers can reuse those same polyfills in their own higher-level compositions built on top of our components, benefiting from features like declarative positioning or invoker patterns beyond what our library provides directly.
...
- **Simplicity**: Less code means faster linting, faster builds, faster tests. Moving to native functionality reduces the surface area we maintain – and every line we don't write is a line that can't break.
- **Easier triage**: When a feature works natively in 80–100% of browsers, we can descope the remaining gap rather than punishing all users equally with a complex JS workaround. Native features let us ship value to the majority or even all in a specific business context immediately and triage the minority as a known limitation — rather than blocking everyone until we've handled every edge case in custom code. Triage isn't pleasant, but with limited resources it's necessary, and native support makes the decision clear-cut.
