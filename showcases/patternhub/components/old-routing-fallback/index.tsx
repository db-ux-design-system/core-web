	...
	useEffect(() => {
		if (router) {
			...
			if (isComponent && component) {
				...
				const allNavigationItems = getAllNavigationItems().sort((a, b) => {
				...
				const foundRoutes = allNavigationItems
					.filter((item) => item.path && item.path?.endsWith(component) && item.path?.split('/').length > 3)
					.sort((a, b) => (a.path?.length ?? 0) - (b.path?.length ?? 0));

				if (foundRoutes.length > 0) {
					router.push(`${foundRoutes[0].path}/${path.join('/')}${pathParameters}`);
				} else {
					router.push('/');
				}
			}
