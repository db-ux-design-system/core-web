...
const CardNavigation = () => {
	...
	const navigationItemParent = getAllNavigationItems().find((navItem) => navItem.path === router.asPath);

	return (
		<DefaultPage noNavigation>
			{navigationItemParent && (
				<div className="card-navigation">
					{navigationItemParent.subNavigation?.map((navItem, index) => (
						<Link key={`navigation-card-${navItem.path}`} href={navItem.path ?? '/'}>
