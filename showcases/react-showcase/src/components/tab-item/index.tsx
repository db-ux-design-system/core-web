...
const getTab = ({ children, active, noText, icon, iconAfter, disabled }: DBTabItemProps) => (
	<DBTabList>
		<DBTabItem active={active} noText={noText} icon={icon} iconAfter={iconAfter} disabled={disabled}>
			{children}
		</DBTabItem>
	</DBTabList>
);
...
const TabItemComponent = (props: BaseComponentProps) => {
	return (
		<DefaultComponent
			...
			variants={getVariants(defaultComponentVariants, getTab, props.slotCode)}></DefaultComponent>
	);
};
