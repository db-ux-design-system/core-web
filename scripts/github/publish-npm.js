...
for (const { dir, name: PACKAGE } of packages) {
	...
	try {
		execSync(
			'pnpm pack --reporter silent --config.ignore-scripts=true --ignore-workspace',
...
for (const step of IS_CI ? ['dry-run', 'provenance'] : ['dry-run']) {
	for (const { dir, name: PACKAGE } of packages) {
		...
		try {
			execSync(
				`pnpm publish --tag ${TAG} ${path.join(buildOutputs, dir, `db-ux-${PACKAGE}-${VALID_SEMVER}.tgz`)} --${step} --no-git-checks --loglevel=debug`,
				{ stdio: 'inherit' }
			);
		} catch (error) {
			console.error(
