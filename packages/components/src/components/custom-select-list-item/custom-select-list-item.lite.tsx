...
export default function DBCustomSelectListItem(props: DBCustomSelectListItemProps) {
	// This is used as forwardRef
	const _ref = useRef<HTMLLIElement | any>(null);
	// jscpd:ignore-start
	const state: DBCustomSelectListItemState = useStore<DBCustomSelectListItemState>({
		_id: undefined,
		hasDivider: false,
	...
	return (
		<li
			...
			<Show when={!props.isGroupTitle} else={<span>{props.groupTitle}</span>}>
				<label
					data-icon={props.type !== 'checkbox' && props.icon ? props.icon : undefined}
