...
The `utils/dialog/ponyfill.ts` fallback covers a `command="request-close"` button that sits **inside** the drawer (the built-in close button and any request-close control you place in the header or content). It does **not** cover an external invoker placed outside the drawer (e.g. `<button command="request-close" commandfor="my-drawer">` elsewhere on the page): its click never reaches the drawer's listener, so in browsers without Invoker Commands support it does nothing. Native support handles external invokers, and for unsupported targets you wire them yourself with the feature-detection fallback shown below.
