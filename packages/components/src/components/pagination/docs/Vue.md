...
number and moves whatever text you wrote into its `aria-label`, so a `<RouterLink>`
reading `Go to the results` is announced as written and displayed as `2`. Provide an
