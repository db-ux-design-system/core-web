...
const testValuesReset = () => {
	test('should clear tags when values prop is set to null', async ({ mount }) => {
		...
		await expect(resetSummary).toContainText('Placeholder');
	});

	test('should clear tags when values prop is set to undefined', async ({ mount }) => {
		// Start with selected values
		const componentWithValues: any = (
			<DBCustomSelect
				options={[{ value: 'Option 1' }, { value: 'Option 2' }]}
	...
	test('should clear tags when values prop is set to empty array', async ({ mount }) => {
		// Start with selected values
		const componentWithValues: any = (
			<DBCustomSelect
				options={[{ value: 'Option 1' }, { value: 'Option 2' }]}
