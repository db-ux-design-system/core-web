...
C7 (Satzpunkte) ist mit A9 und A3 vollständig abgedeckt und deshalb aus der zurückgestellten Liste entfernt. Mit A14 ist zusätzlich der Kommentar-Teilpunkt von [A13](#a13--struktur-nicht-garantiert) entfallen: Die Extraktion nach „erste Zeile nach H1" liefert jetzt überall die Beschreibung statt bei `navigation` einen Kommentar. Beim Umformulieren im Zuge von A1 und A6 sind außerdem zwei A12-Fundstellen (`custom-select-form-field:9`, `custom-select-dropdown:8`) und eine C6-Fundstelle weggefallen.
...
- `infotext:11` „Hervorhebungen sparsam einsetzen" → Stilhinweis
...
- `backdrop:13` — „Klick auf den Backdrop kann das Overlay schließen (optional)": optional ist das Verhalten, nicht der Klick — nachgestelltes „(optional)" lässt beides zu. Steht seit A6 unter `## Zusätzliche Informationen`.
