...
2. Die erste und letzte Seite immer anzeigen, `boundaryCount` also auf mindestens `1` setzen. Bei `0` steht an den Rändern eine Truncation statt der ersten und letzten Seite, und der Umfang des Datensatzes ist nicht erkennbar.
3. **sollte** die Pagination einzeilig halten und `boundaryCount` und `siblingCount` so wählen, dass keine zweite Zeile entsteht. Höchstens sieben Seitenzahlen sind dafür der Richtwert; mehrzeilig wird die Pagination schwerer zu bedienen.
4. Für jede Lücke in der Seitenfolge eine Truncation anzeigen. Ohne sie lesen sich die beiden Seitenzahlen als aufeinanderfolgend.
5. Als aktuelle Seite die Seite markieren, deren Inhalt angezeigt wird. Eine falsche Markierung sieht korrekt aus und fällt an der Pagination selbst nicht auf.

## Zusätzliche Informationen

- `pageSize` bestimmt, wie viele Einträge auf eine Seite kommen; der Default ist `10`. Welcher Wert passt, hängt von der Art der Einträge und ihrer Anordnung ab: Tabellenzeilen tragen mehr pro Seite als mehrzeilige Karten, ein mehrspaltiges Grid mehr als eine einspaltige Liste. _(Example-Kandidat)_
