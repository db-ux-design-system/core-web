...
- **`type` ist Figma-only, im Code ist die Truncation die Abwesenheit von `page`** — Figma unterscheidet Seite und Truncation über die Achse `🔀 Type` mit `(Def) Page` und `Truncation` innerhalb von Pagination Item. Im Code gibt es kein `type`; ein `DBPaginationItem` ohne `page` rendert die Truncation. Konsequenz für die Doku: Die Truncation ist am Item eine reine Design-Property. Im Code setzt `DBPagination` sie automatisch aus den Lücken der Seitenfolge, niemand konfiguriert sie am Item — deshalb beschreibt die Item-Doku sie nicht als Aufgabe des Items, sondern führt die Regel dazu an der Pagination. Anders als bei einer getrennten Komponente ist die Unterscheidung damit auf beiden Seiten dieselbe Komponente und für Code Connect abbildbar, benötigt dafür aber ein Template statt einer Enum-Zuordnung, weil `Truncation` bedeutet, `page` weglassen.
