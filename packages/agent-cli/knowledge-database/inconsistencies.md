...
- **Slot-Komponente vs. berechnete Items** — In Figma ist Pagination eine Slot-Komponente: die beiden Pfeile rahmen `Children` ein, und die Designer legen die Items je nach Konfiguration selbst hinein. Im Code berechnet `DBPagination` die Items aus `currentPage`, `totalCount`, `pageSize`, `siblingCount` und `boundaryCount`. Konsequenz: Die Datenfläche der Komponente ist code-only und aus Figma nicht ableitbar. Umgekehrt ist die Reihe in Figma frei zusammenstellbar, auch in Reihenfolgen und Mengen, die der Code nie erzeugt. Der Slot begrenzt nur, welche Komponente einsetzbar ist, und verlangt mindestens zwei Items; welche Seitenzahlen in welcher Folge stehen, prüft er nicht.
- **Truncation ist in Figma eine eigene Helper-Komponente, im Code die Abwesenheit von `page`** — In Figma ist die Truncation eine eigenständige Helper-Komponente `🛟 Pagination Truncation` (je eine Medium- und Small-Variante), die neben den Pagination Items in den Children-Slot der Pagination gelegt wird. Im Code gibt es keine solche Komponente: `DBPagination` erzeugt die Truncation, indem es an einem Item die `page` weglässt und auf der angrenzenden Seite eine Ellipse (`data-ellipsis-*`) zeichnet. Sie leitet sich dort automatisch aus den Lücken der Seitenfolge ab und wird nie von Hand gesetzt. Konsequenz: 1:0-Mapping — die Figma-Helper-Komponente hat kein Code-Pendant, und für Code Connect braucht die Truncation ein eigenes Template statt einer Property-Zuordnung. Helper-Komponenten werden nicht eigens dokumentiert, deshalb führt die Item-Doku die Regel zur Truncation an der Pagination. (Zuvor war die Truncation als Achse `🔀 Type` am Pagination Item modelliert; diese Achse ist entfallen.)
- **`position` ist Figma-only** — Die Achse `(Def) Start` / `Center` / `End` beschreibt, wo die aktuelle Seite im Datensatz liegt. Im Code existiert kein `position`; die Lage folgt aus `currentPage` im Verhältnis zu `totalCount` und `pageSize`.
- **`size` hat in Figma keine Property, sondern getrennte Komponenten auf beiden Ebenen** — `Pagination - (Def) Medium` und `Pagination - Small` sind zwei Hauptkomponenten, ebenso `↳ Pagination Item - Medium` und `↳ Pagination Item - Small`. Im Code ist es je eine geteilte `size`-Prop. 1:2-Mapping auf beiden Ebenen, und die Größe wird über Code Connect an der jeweiligen Komponente übergeben, nicht über eine Figma-Property. Die Aufteilung ist gewollt: Nur so kann der Children-Slot die Einsetzbarkeit auf das Item-Set der passenden Größe begrenzen. Sie folgt dem Muster von Link, Infotext und Checkbox. Die Truncation ist eine eigene Helper-Komponente und ebenfalls in einer Medium- und einer Small-Variante ausgelegt.
