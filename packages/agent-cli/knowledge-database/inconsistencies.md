...
- **Truncation ist in Figma eine eigene Helper-Komponente, im Code die Abwesenheit von `page`** — In Figma wird die Helper-Komponente `🛟 Pagination Truncation` (Medium und Small) neben den Items in den Children-Slot gelegt. Im Code gibt es kein Pendant: `DBPagination` lässt an einem Item die `page` weg und zeichnet auf der angrenzenden Seite eine Ellipse (`data-ellipsis-*`), abgeleitet aus den Lücken der Seitenfolge. Konsequenz: 1:0-Mapping, und für Code Connect braucht die Truncation ein eigenes Template statt einer Property-Zuordnung.
