...
- **`type` ist Figma-only, im Code ist die Truncation die Abwesenheit von `page`** — Figma unterscheidet Seite und Auslassung über die Achse `🔀 Type` mit `(Def) Page` und `Truncation` innerhalb von Pagination Item. Im Code gibt es kein `type`; ein `DBPaginationItem` ohne `page` rendert die Truncation. Anders als bei einer getrennten Komponente ist die Unterscheidung damit auf beiden Seiten dieselbe Komponente und für Code Connect abbildbar, benötigt dafür aber ein Template statt einer Enum-Zuordnung, weil `Truncation` bedeutet, `page` weglassen.
