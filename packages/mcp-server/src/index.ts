...
server.registerPrompt(
	"scaffold_page",
	{
		...
		argsSchema: {
			page_type: z.string().describe("The functional domain or structural type of the page (e.g. 'Login Portal', 'Dashboard', 'Settings Form'). Used in Phase 1 to decompose the page into UI blocks and drives component selection throughout the workflow."),
			framework: z.string().describe("The target framework for code generation. Allowed values: angular, react, vue, web-components, html. Must be passed verbatim as the 'framework' parameter to every get_example_code call."),
			additional_requirements: z.string().optional().describe("Optional functional or architectural requirements (e.g. 'must include a data table with pagination', 'dark mode support required'). These requirements refine the UI block decomposition in Phase 1 and may add or remove components from the plan."),
		},
	},
	({ page_type, framework, additional_requirements }) => ({
		description: "Builds a DB UX page layout using the full MCP discovery workflow",
		messages: [
			{
				role: "user",
				content: {
