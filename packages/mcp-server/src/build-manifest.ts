 ...
 * when invoked via `npx @db-ux/core-components`).
 */
...
async function buildManifest() {
	...
	const allIconsSrc = await readOptional(join(FOUNDATIONS_SRC, "all-icons.ts"));
	...
	for (const name of componentNames) {
		...
		for (const fw of FRAMEWORKS) {
			exampleCode[fw] = {};
			const exDir = join(OUTPUT_DIR, fw, "src/components", name, "examples");
