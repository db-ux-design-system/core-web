...
/**
 * Instead of running a hardcoded compiler command (tsc/vue-tsc), this tool
 * returns a textual instruction for the LLM to determine the appropriate
 * verification strategy from the project's own toolchain.
 *
 * This avoids assumptions about TypeScript, Vite, or any specific build tool.
 */
export async function handleVerifyMigratedCode({
	...
	const instruction = [
		`You generated ${framework} code that needs verification.`,
		'',
