...
export async function handleListComponents(): Promise<ToolResult> {
	...
	return {
		content: [{ type: 'text', text: truncate(JSON.stringify(Object.keys(manifest.components), null, 2), MAX_JSON_OUTPUT) }]
	};
}
...
export async function handleGetComponentDetails({ componentName }: { componentName: string }): Promise<ToolResult> {
	if (IS_MONOREPO) {
		...
		return {
			content: [{ type: 'text', text: examples.length > 0 ? JSON.stringify(examples, null, 2) : 'No examples found.' }]
		};
	}
	const manifest = await getManifest();
	...
	return {
		content: [{ type: 'text', text: comp.examples.length > 0 ? JSON.stringify(comp.examples, null, 2) : 'No examples found.' }]
	};
}
...
export async function handleGetComponentProps({ componentName }: { componentName: string }): Promise<ToolResult> {
	if (IS_MONOREPO) {
		...
		if (!existsSync(modelFile)) {
			return {
				content: [{ type: 'text', text: `Error: Props file (model.ts) for component '${componentName}' not found.` }],
				isError: true
			};
		}
		return { content: [{ type: 'text', text: truncate(await readFile(modelFile, 'utf-8'), MAX_FILE_CONTENT) }] };
	}
	const manifest = await getManifest();
	const comp = manifest.components[componentName];
	if (!comp) {
			...
			try {
				...
				if (IS_MONOREPO) {
					if (framework === 'html') {
						...
						return { content: [{ type: 'text', text: truncate(await readFile(htmlFile, 'utf-8'), MAX_FILE_CONTENT) }] };
					}
