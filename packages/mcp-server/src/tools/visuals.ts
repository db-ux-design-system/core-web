...
import { type ToolResult, error, getAssetsDir } from '../utils';
 ...
 */
const visualsDir = () => join(getAssetsDir(), 'visuals');
...
/**
 ...
 Returns a pre-optimised visual reference image (max 800x800 px, JPEG q75)
	...
	try {
		...
		return {
			content: [
				...
				{
					type: 'text',
					text: `Visual reference for '${name}' (pre-optimised: max 800x800 px, JPEG q75).`
				}
			]
		};
