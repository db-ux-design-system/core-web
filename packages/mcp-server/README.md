...
| `get_visual_reference`         | Returns a pre-optimised visual reference image (max 800×800 px, JPEG q75) as a Base64-encoded MCP image block. Use this when visual context is needed for complex layouts. No native dependencies (`sharp`) at runtime — images are optimised at build time.                                                                                                                                                     |
...
>
> **Note on visual references:** The `get_visual_reference` tool serves pre-optimised JPEG images (max 800×800 px, q75) from `assets/visuals/`. Images are downsampled at build time using `sharp` (devDependency only) — zero native dependencies at runtime.
