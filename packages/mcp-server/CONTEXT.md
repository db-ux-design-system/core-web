...
| `scan_generation_2_migration`  | Scans a file for DB UX Design System – Generation 2 patterns (components, colors, icons) and returns a JSON report with line numbers and deterministic migration suggestions. Call FIRST before migrating. (Old name `scan_v2_migration` still works as a deprecated alias.) |
