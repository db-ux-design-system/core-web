...
| `packages/components/src` — **component logic, properties or consumer-visible markup**: `model.ts`, state/handlers/effects, changed accessibility semantics (roles, labels, headings, landmarks), a different visual result, or DOM structure consumers target with custom CSS, tests or query selectors | `@db-ux/ngx-core-components`, `@db-ux/react-core-components`, `@db-ux/wc-core-components`, `@db-ux/v-core-components`                                                      |
