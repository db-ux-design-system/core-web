...
A stable, semantic class used as a **stylesheet hook** is allowed and often required — e.g. `button.lite.tsx` carries `db-button` so `button.scss` can select it. The test is the name: a class that identifies what the element _is_ or its state (`db-button`, `db-button--loading`) is fine; a class that names how it should _look_ is not. Prefer such a hook over a brittle structural selector.
