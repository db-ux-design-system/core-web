...
| `packages/components/src` — **component logic, properties or consumer-visible markup** changes: `model.ts`, state/handlers/effects, changed accessibility semantics (roles, labels, headings, landmarks), a different visual result, or DOM structure consumers target with custom CSS, tests or query selectors | `@db-ux/ngx-core-components`, `@db-ux/react-core-components`, `@db-ux/wc-core-components`, `@db-ux/v-core-components`                                                      |
| `packages/components/src` — **only the template**: inner HTML restructuring in a Mitosis component file that keeps behavior, properties, visual result and accessibility tree identical                                                                                                                  | `@db-ux/core-components`                                                                                                                                                   |
...
**Template-only changes stay on `@db-ux/core-components`.** Mitosis does compile the markup into every framework target, so the framework packages technically change too. We still do not list them: a changelog entry answers "what changes for me?", and the answers worth an entry are behavior, API and visual result — a pure markup restructuring delivers none of them, the inner HTML is an implementation detail the framework component abstracts away. Everything a consumer _can_ observe is covered by the row above, so use that row whenever you are in doubt.
