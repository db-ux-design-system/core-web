...
for PACKAGE in 'foundations' 'migration' 'stylelint' 'components' 'ngx-components' 'react-components' 'v-components' 'web-components'; do
	echo "Start $PACKAGE bundle:"

	echo "🆚 Update Version"
	npm version --no-git-tag-version "$VALID_SEMVER_VERSION" --workspace=@db-ui/"$PACKAGE"

	if [[ $PACKAGE != 'foundations' && $PACKAGE != 'migration' && $PACKAGE != 'stylelint' ]]; then
