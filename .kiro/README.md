...
6. **Set issue type** — Assigns Bug, Task, or Story based on content when no type is set yet **and** completeness is ⚠️ Partial or ✅ Complete. Incomplete reports are left untyped to avoid locking in a wrong guess. On re-triage, an **existing AI-assigned type is updated** if newly provided information contradicts it; types set by humans are left alone.
